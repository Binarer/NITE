...
class TaskFormController extends GetxController {
  ...
  void _loadTask(TaskModel task) {
    ...
    // Граммы: из foodItemGrams (новое поле) или fallback на foodGrams (обратная совместимость)
    foodItemGrams.clear();
    for (final id in foodItemIds) {
      foodItemGrams[id] = task.foodItemGrams.isNotEmpty
  ...
  Future<void> saveTask() async {
    ...
    try {
      ...
      final gramsMap = Map<String, double>.from(foodItemGrams);
